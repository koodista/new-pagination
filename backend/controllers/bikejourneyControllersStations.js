const BikejourneyStations = require('../models/BikejourneyStations')
...
exports.getAllBikejourneysStations = asyncHandler(async (req, res, next) => {
  //const bikejourneys = await Bikejourney.find();

  let query = BikejourneyStations.find();

  const page = parseInt(req.query.page) || 1;
  const pageSize = parseInt(req.query.limit) || 50;
  const skip = (page - 1) * pageSize;
  const total = await BikejourneyStations.countDocuments();
...
exports.filterAllBikejourneysStations = asyncHandler(async (req, res, next) => {    //mongodb query language
  ...
  query = BikejourneyStations.find(JSON.parse(queryStr));
  ...
  const bikejourneysStations = await query;
  
  res.status(200).json({
    status: 'success',
    data: bikejourneysStations
  })
});

exports.createNewBikejourneyStations = asyncHandler(async (req, res, next) => {
  const bikejourney = await BikejourneyStations.create(req.body);

  res.status(201).json({
    success: true,
    data: bikejourneyStations
  })
});

exports.updateBikejourneyByIdStations = asyncHandler(async (req, res, next) => {
  let bikejourneyStations = await BikejourneyStations.findById(req.params.id);

  if(!bikejourneyStations) {
    return next(new ErrorResponse(`Journey with id ${req.params.id} was not found`, 404))
  }

  bikejourney = await BikejourneyStations.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true});
...
exports.deleteBikejourneyByIdStations = asyncHandler(async (req, res, next) => {
  let bikejourneyStations = await BikejourneyStations.findById(req.params.id);
  ...
  await bikejourneyStations.remove();
